...
// 定义连接助手对象，用于redis和mongodb连接
object ConnHelper extends Serializable {
  lazy val jedis = new Jedis("localhost")
  lazy val mongoClient = MongoClient(MongoClientURI("mongodb://192.168.0.111:27017/recommender"))
}
...
object OnlineRecommender {
  ...
  def main(args: Array[String]): Unit = {
    ...
    // 创建kafka配置参数
    val kafkaPara = Map("bootstrap.servers" -> "192.168.0.111:9092",
    ...
    // kafka接收的消息userId|productId|score|timestamp
      ...
      rdds =>
        rdds.foreach {
          case (userId, productId, score, timestamp) =>
            ...
            val userRecentlyRatings = getUserRecentlyRatings(MAX_SIM_PRODUCT_NUM, userId, ConnHelper.jedis)
            // 2.从相似度矩阵中获取当前商品最相似的商品列表，作为备选列表Array[productId]
            val candidateProducts = getTopSimProducts(MAX_SIM_PRODUCT_NUM, productId, userId, simProductsMatrixBC.value)
            // 3.计算每个备选商品的推荐优先级,得到当前用户的实时推荐列表,保存成数组Array[(productId,score)]
  ...
  def getUserRecentlyRatings(num: Int, userId: Int, jedis: Jedis): Array[(Int, Double)] = {
    // 从redis中用户的评分队列获取评分数据,key为 uid:userId,eg: uid:1，value为productId：score， eg：1：2
  ...
  def getTopSimProducts(num: Int,
                        productId: Int,
                        userId: Int,
                        simProduct: scala.collection.Map[Int, Map[Int, Double]])
                       (implicit mongoConfig: MongoConfig): Array[Int] = {
    // 从广播变量相似度矩阵中拿到当前商品相似度列表
    val allSimProducts = simProduct(productId).toArray
    // 获得用户已评分过的商品，过滤掉，排序输出
  ...
  def computeProductScore(candidateProducts: Array[Int],
                          ...
                          simProduct: scala.collection.Map[Int, Map[Int, Double]]): Array[(Int, Double)] = {
    // 优先级计算公式
    ...
    for (candidateProduct <- candidateProducts; userRecentlyRating <- userRecentlyRatings) {
      ...
      val simScore = getProductsSimScore(candidateProduct, userRecentlyRating._1, simProduct)
      if (simScore > 0.4) {
        // 按照公式进行加权计算，得到基础评分
